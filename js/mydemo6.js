 ...
 * 默认为 #ea80b0
 */
function renderLxHeart(canvas, heartColor) {
  ...
  var image = (function () {
    ...
    context.closePath();
    // create the heart fill
    // '#ff30c5' '#ea80b0'
    context.fillStyle = heartColor || '#ea80b0';
    context.fill();
    // create the image
    var image = new Image();
...
function fillText(canvas, text) {
  ...
  function Heart() {
    ...
    this.vel = {
      x: (Math.random() - 0.5) * 8, // 大幅提高移动速度
      y: (Math.random() - 0.5) * 8, // 大幅提高移动速度
    };
    this.targetScale = Math.random() * 0.3 + 0.1; // 大幅增加缩放比例
  ...
  Heart.prototype.update = function (i) {
    ...
    this.scale += (this.targetScale - this.scale) * 0.03; // 大幅加快缩放速度
  ...
  Heart.prototype.draw = function (i, text) {
    ...
    ctx.font = `${220 * this.scale}px "微软雅黑"`; // 增大字体大小
    // ctx.font="20px";
    ctx.fillStyle = colorss[i % 20];
    ctx.fillText(
      text,
